...
class DashboardController extends Controller
{
    ...
    {
        $user = Auth::user();
        $wallet_transactions = WalletTransaction::where('user_id', $user->id)->orderByDesc('id')->paginate(10);
    ...
    public function withdraw_wallet_store(StoreWithdrawWalletRequest $request) {
        ...
        if($user->wallet->balance < 100000) {
            return redirect()->back()->withErrors([
                'amount' => 'Balance Anda tidak cukup untuk melakukan penarikan.'
            ]);
        }
        DB::transaction(function () use ($request, $user) {
            ...
            $newWithdraw = WalletTransaction::create($validated);
    ...
    public function topup_wallet_store(StoreTopupWalletRequest $request)
    {
        ...
        DB::transaction(function () use ($request, $user) {
            ...
            $newTopup = WalletTransaction::create($validated);
        });
